...
var Subatomic = {
  createElement: function createElement(type) {
    ...
    return "<".concat(type, " ").concat(Object.keys(props).map(function (a) {
      return "".concat(a, "=\"").concat(props[a], "\"");
    }).join(" "), ">").concat(children.join(""), "</").concat(type, ">");
  }
};
function render(build, data) {
  ...
  var ui = [Subatomic.createElement("div", null, "\n      ", Subatomic.createElement("h1", null, "Test"), "\n      ", Subatomic.createElement("button", null, "\n        pres me\n      "), "\n    ")];
  if (typeof document !== "undefined") {
    useEffect(function () {
      console.log("use effect to run side effects");
    }, ["cookies"]);
    document.body.innerHTML = "<body>".concat(parseArray(ui), "</body>");
    asdf.value = variables.input;
    asdf.onchange = function () {
      variables.input = asdf.value;
    };
    Object.keys(variables).forEach(function (a) {
...
state();
function Cookies() {
  return "<h1>".concat(variables.cookies, "</h1>");
}
function state() {
  variables.cookies = 0;
  variables.todos = [];
